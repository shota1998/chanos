...
void SwitchEhci2Xhci(const pci::Device& xhc_dev) {
  ...
  uint32_t surperspeed_ports = pci::ReadConfReg(xhc_dev, 0xdc);
  pci::WriteConfReg(xhc_dev, 0xd8, surperspeed_ports);
  uint32_t ehci2xhci_ports = pci::ReadConfReg(xhc_dev, 0xd4);
  pci::WriteConfReg(xhc_dev, 0xd0, ehci2xhci_ports);
  Log(kDebug, "SwitchEhci2Xhci: SS = %02, xHCI = %02x\n",
      surperspeed_ports, ehci2xhci_ports);
}
...
extern "C" void KernelMain(const FrameBufferConfig& frame_buffer_config) {
  ...
  for (int i = 0; i < pci::num_device; ++i) {
    ...
    auto vendor_id = pci::ReadVenderId(dev);
